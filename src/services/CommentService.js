...
const { findProduct } = require("./ProductService-V2");
...
class CommentService {
  static async createComment({
    ...
    if (parentCommentId) {
      ...
      // xoá comment (update many comments)
  ...
  static async deleteComment({ productId, commentId }) {
    ...
    // 1.xác định left-right
    const comment = await Comment.findById(commentId);

    if (!comment) throw new NotFoundError("Comment not found");
    const leftValue = comment.comment_left;
    const rightValue = comment.comment_right;
    // 2. tinh width comments đang xoá (all: left, right)
    const width = rightValue - leftValue + 1;

    // 3. delete all parentCommentId
    await Comment.deleteMany({
      comment_productId: convertToObjectId(productId),
      comment_left: { $gte: leftValue, $lte: rightValue },
    });

    // 4. update value left and right
    await Comment.updateMany(
      {
        comment_productId: convertToObjectId(productId),
